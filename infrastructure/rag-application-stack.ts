...
Respond with a score between 0 and 1, a brief explanation, and list any false positives or missed anomalies.`;
    ...
    // The L2 construct generates correct permissions, but CloudFormation creates the role
    // and OnlineEvaluationConfig simultaneously — the service validates before IAM propagates.
    ...
    onlineEvalRole.addToPrincipalPolicy(new iam.PolicyStatement({
      ...
      resources: [cdk.Arn.format({ service: 'logs', resource: 'log-group', resourceName: '/aws/bedrock-agentcore/evaluations/results/*', arnFormat: cdk.ArnFormat.COLON_RESOURCE_NAME }, this)],
    }));
    onlineEvalRole.addToPrincipalPolicy(new iam.PolicyStatement({
