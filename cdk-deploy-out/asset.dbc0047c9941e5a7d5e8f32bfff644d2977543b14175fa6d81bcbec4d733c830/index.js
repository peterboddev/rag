...
// src/lambda/document-summary-selective.ts
var document_summary_selective_exports = {};
__export(document_summary_selective_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(document_summary_selective_exports);
...
var handler = async (event) => {
  try {
    ...
    const includedDocumentRefs = processedDocuments.map((doc) => ({
      documentId: doc.documentId,
      fileName: doc.fileName,
    ...
    const response = {
      ...
      excludedDocuments: excludedDocuments.length > 0 ? excludedDocuments : void 0
    };
...
async function getSelectedDocuments(customerUUID, documentIds, tenantId) {
  try {
    const documents = [];
    for (const documentId of documentIds) {
      try {
        const result = await dynamoClient.send(new import_lib_dynamodb2.GetCommand({
...
async function generateSelectiveDocumentSummary(processedContent, customerEmail, tokenUsage, truncationInfo, documentCount) {
  try {
...
4. Any notable patterns or relationships between the selected documents`;
    ...
    if (documentCount > 5) {
      ...
      summaryLength = summaryLength.replace(
        /(\d+)-(\d+)/,
        (match, min, max) => `${Math.floor(parseInt(min) * 1.2)}-${Math.floor(parseInt(max) * 1.2)}`
      );
    } else if (documentCount === 1) {
      maxNewTokens = Math.floor(maxNewTokens * 0.8);
      summaryLength = summaryLength.replace(
        /(\d+)-(\d+)/,
        (match, min, max) => `${Math.floor(parseInt(min) * 0.8)}-${Math.floor(parseInt(max) * 0.8)}`
      );
    }
    prompt += `
